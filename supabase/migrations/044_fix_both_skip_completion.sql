-- 044: Fix both-skip bypass in complete_date_atomic for outside dates.
--
-- Bug: the dual check-in guard checked only that checkin_owner_at or
-- checkin_partner_at was non-null. skipCheckIn() sets those timestamps
-- even when the partner skips, so both-skipped outside dates passed the
-- guard. Additionally, when both skip, v_member_count = 0 and the photo
-- count check (0 < 0 = false) would silently pass.
--
-- Fix 1: tighten the guard to require at least one physically-present
--        (non-skipped) check-in for outside dates.
-- Fix 2: add an explicit v_member_count = 0 guard as defense in depth.
...
begin
  ...
  -- skipCheckIn() sets checkin_*_at even for skipped partners, so we must also
  -- exclude rows where checkin_*_skipped = true.
  ...
  if v_location_type = 'home' then
    ...
    -- Defense in depth: if both partners skipped, the check-in guard above
    -- should have already caught this. Belt-and-suspenders.
