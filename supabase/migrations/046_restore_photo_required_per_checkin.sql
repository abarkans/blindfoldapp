-- 046: Restore outside-date photo prerequisite from migration 044.
--
-- Migration 045 relaxed the guard so that a check-in alone counted as
-- "decided", allowing completion before the partner had uploaded or skipped
-- a photo. That was incorrect.
--
-- Rule: every member who physically checked in (not skipped) must have a
-- date_photos row (upload OR skip) before the date can complete.
...
begin
  ...
  if v_location_type is distinct from 'home' then
    -- At least one partner must have physically checked in (not skipped).
    if not exists (
    ...
    -- Every member who physically checked in must have a photo row.
