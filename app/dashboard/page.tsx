...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: latestDateIdea }] = await Promise.all([
    admin.from("profiles").select("id, partner_names, interests, constraints, plan_type, cadence, date_idea, date_teaser, last_lat, last_long, preferred_radius, onboarding_complete, revealed_at, total_rerolls_used, current_date_rerolled, date_accepted_at, reveal_owner_ready_at, reveal_partner_ready_at, total_xp, dates_completed_count, subscription_ends_at, notification_sent_at, stripe_customer_id, checkin_owner_at, checkin_partner_at, total_checkins, created_at, updated_at").eq("id", access.profileId).single(),
    admin
      .from("date_ideas")
      .select("id, status")
