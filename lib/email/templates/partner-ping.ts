interface PartnerPingEmailProps {
  partnerName: string;
...
export function partnerPingEmail({
  partnerName,
  unsubscribeUrl,
}: PartnerPingEmailProps): { subject: string; html: string } {
  const safeName = escapeHtml(partnerName || "Your partner");
  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "https://blindfolddate.com";

  return {
    subject: `${safeName} can't wait for date night`,
    html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:420px;">
        ...
        <tr><td style="background:rgba(255,255,255,0.035);border:1px solid rgba(255,255,255,0.16);border-radius:24px;box-shadow:0 28px 80px rgba(0,0,0,0.45);padding:32px 28px;text-align:center;">
          ...
          <h1 style="margin:0 0 12px;font-size:22px;line-height:1.25;color:#ffffff;">${safeName} can't wait for date night.</h1>
          <p style="margin:0 0 28px;font-size:15px;line-height:1.6;color:rgba(255,255,255,0.62);">
            They've already tapped I'm ready. Once you do the same, the full surprise will reveal for both of you.
          </p>
          <a href="${appUrl}/dashboard?tab=date" style="display:inline-block;padding:14px 28px;background:#f43f5e;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;border-radius:999px;">See the teaser &rarr;</a>
        </td></tr>
