...
export function reengagementEmail({
  ...
  partner2,
  unsubscribeUrl,
}: ReengagementEmailProps): { subject: string; html: string } {
  ...
  const heading = hasPartner
    ? `${safePartner1} &amp; ${safePartner2}, it's been a while.`
...
<head>
  ...
  <title>Date night is calling</title>
          ...
          <tr>
            <td style="background:rgba(255,255,255,0.035);border:1px solid rgba(255,255,255,0.16);border-radius:24px;box-shadow:0 28px 80px rgba(0,0,0,0.45);padding:32px 28px;text-align:center;">
              ...
              <p style="margin:0 0 28px;font-size:15px;line-height:1.6;color:rgba(255,255,255,0.62);">
                Life gets busy — we get it. But your next mystery date is just one tap away. Let us plan something special and get you ${hasPartner ? "two" : ""} back out there.
              </p>
